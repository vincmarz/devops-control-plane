...
func TestNewVerifiesTLSByDefault(t *testing.T) {
	c, err := New(Config{BaseURL: "https://gitlab.example.test", Token: "test-token"})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func TestNewDisablesTLSVerificationOnlyWhenRequested(t *testing.T) {
	c, err := New(Config{BaseURL: "https://gitlab.example.test", Token: "test-token", InsecureTLS: true})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
