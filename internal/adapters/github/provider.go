...
func (p *Provider) CreateOrUpdateFile(ctx context.Context, target app.GitRepositoryTarget, branch, filePath, commitMessage, content string) (app.GitFileUpdateResult, error) {
	...
	updated, err := p.client.CreateOrUpdateFile(ctx, target.ProjectPath, branch, filePath, commitMessage, content)
	if err != nil {
		return app.GitFileUpdateResult{}, err
	}
	return app.GitFileUpdateResult{FilePath: filePath, Branch: branch, BlobSHA: updated.SHA, CommitSHA: updated.CommitSHA}, nil
}
