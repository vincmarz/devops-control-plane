...
</style></head><body><div class="app"><aside class="sidebar"><div class="brand"><div class="brand-icon">☁</div><div>DevOps Control Plane</div></div><nav class="nav"><a class="{{if eq .Active "dashboard"}}active{{end}}" href="/">▣ Dashboard</a><a href="/ui/applications">▧ Applications</a><div class="nav-group"><a class="{{if eq .Active "changes"}}active{{end}}" href="/ui/changes">◌ Change Requests</a><a href="/ui/changes">All changes</a><a href="/api/v1/changes">New change</a></div><div class="nav-group"><a href="/ui/changes/CHG-2026-0005">▤ Evidence</a><a href="/ui/changes/CHG-2026-0005">☷ Audit Log</a></div><div class="nav-group"><a href="/readyz">⚙ Settings</a></div></nav><div class="sys"><h4>System status</h4><div class="sys-row"><span><i class="dot"></i>API</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Database</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>GitLab</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Tekton</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Argo CD</span><b>OK</b></div></div><div class="version">DevOps Control Plane<br>v0.1.0</div></aside><main class="main"><header class="topbar"><div class="title"><h1>{{.Title}}</h1><p>{{.Subtitle}}</p></div><div class="user"><div class="select">Environment<br><b>dev</b></div><div class="avatar">A</div><b>admin</b></div></header><section class="content">{{if .Flash}}<div class="alert alert-ok">{{.Flash}}</div>{{end}}{{if .ActionError}}<div class="alert alert-error">{{.ActionError}}</div>{{end}}{{if .Error}}<div class="card detail"><b>Error:</b> {{.Error}}</div>{{else}}{{if eq .Active "changes"}}{{if .SelectedChange}}{{template "changeDetail" .}}{{else}}{{template "changesList" .}}{{end}}{{else if eq .Active "applications"}}{{if .SelectedApplication}}{{template "applicationDetail" .}}{{else}}{{template "applicationsList" .}}{{end}}{{else}}{{template "dashboard" .}}{{end}}{{end}}<div class="footer">© 2026 DevOps Control Plane <span style="float:right">v0.1.0</span></div></section></main></div></body></html>
