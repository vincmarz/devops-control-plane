...
func (h *Handler) renderUI(w http.ResponseWriter, status int, data uiData) {
	funcs := template.FuncMap{"get": get, "str": str, "short": short, "badgeClass": badgeClass, "latestEvidence": latestEvidence, "kubeSummary": kubeSummary, "eventStep": eventStep, "jsonPretty": jsonPretty, "changeNumberOrID": changeNumberOrID, "recommendedActions": recommendedActions, "advancedActions": advancedActions, "applicationName": applicationName}
...
</style></head><body><div class="app"><aside class="sidebar"><div class="brand"><div class="brand-icon">☁</div><div>DevOps Control Plane</div></div><nav class="nav"><a class="{{if eq .Active "dashboard"}}active{{end}}" href="/">▣ Dashboard</a><a class="{{if eq .Active "applications"}}active{{end}}" href="/ui/applications">▧ Applications</a><div class="nav-group"><a class="{{if eq .Active "changes"}}active{{end}}" href="/ui/changes">◌ Change Requests</a><a href="/ui/changes">All changes</a><a href="/api/v1/changes">Changes API</a></div><div class="nav-group"><a href="/ui/changes/CHG-2026-0005/evidence">▤ Evidence</a><a href="/ui/changes/CHG-2026-0005/events">☷ Audit log</a></div><div class="nav-group"><a class="{{if eq .Active "settings"}}active{{end}}" href="/ui/settings">⚙ Settings</a></div></nav><div class="sys"><h4>System status</h4><div class="sys-row"><span><i class="dot"></i>API</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Database</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>GitLab</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Tekton</span><b>OK</b></div><div class="sys-row"><span><i class="dot"></i>Argo CD</span><b>OK</b></div></div><div class="version">DevOps Control Plane<br>v0.1.0</div></aside><main class="main"><header class="topbar"><div class="title"><h1>{{.Title}}</h1><p>{{.Subtitle}}</p></div><div class="user"><div class="select">Environment<br><b>dev</b></div><div class="avatar">A</div><b>admin</b></div></header><section class="content">{{if .Flash}}<div class="alert alert-ok">{{.Flash}}</div>{{end}}{{if .ActionError}}<div class="alert alert-error">{{.ActionError}}</div>{{end}}{{if .Error}}<div class="card detail"><b>Error:</b> {{.Error}}</div>{{else}}{{if eq .Mode "changeEvents"}}{{template "changeEventsPage" .}}{{else if eq .Mode "changeEvidence"}}{{template "changeEvidencePage" .}}{{else if eq .Active "settings"}}{{template "settingsPage" .}}{{else if eq .Active "changes"}}{{if .SelectedChange}}{{template "changeDetail" .}}{{else}}{{template "changesList" .}}{{end}}{{else if eq .Active "applications"}}{{if .SelectedApplication}}{{template "applicationDetail" .}}{{else}}{{template "applicationsList" .}}{{end}}{{else}}{{template "dashboard" .}}{{end}}{{end}}<div class="footer">© 2026 DevOps Control Plane <span style="float:right">v0.1.0</span></div></section></main></div></body></html>
{{define "dashboard"}}<div class="cards"><div class="card metric"><div class="icon" style="background:#dbeafe;color:#2563eb">□</div><div>Applications<b>{{get .Stats "applications"}}</b><span>Managed total</span></div></div><div class="card metric"><div class="icon" style="background:#dcfce7;color:#16a34a">✓</div><div>Completed changes<b>{{get .Stats "completed"}}</b><span>Last 30 days</span></div></div><div class="card metric"><div class="icon" style="background:#fef3c7;color:#d97706">◷</div><div>Running changes<b>{{get .Stats "running"}}</b><span>Currently running</span></div></div><div class="card metric"><div class="icon" style="background:#fee2e2;color:#dc2626">!</div><div>Failed changes<b>{{get .Stats "failed"}}</b><span>Last 30 days</span></div></div><div class="card metric"><div class="icon" style="background:#ede9fe;color:#7c3aed">▤</div><div>Collected evidence<b>{{get .Stats "evidence"}}</b><span>For selected change</span></div></div></div><div class="grid"><div><div class="card panel"><h3>Applications <a style="float:right;font-size:13px" href="/ui/applications">View all</a></h3><ul class="list">{{range .Applications}}<li><div><a href="/ui/applications/{{get . "name"}}"><b>{{get . "name"}}</b></a><div class="small">{{get . "targetNamespace"}}</div></div><span class="badge {{badgeClass (get . "healthStatus")}}">{{get . "healthStatus"}}</span></li>{{end}}</ul></div><div class="card panel" style="margin-top:18px"><h3>Recent changes <a style="float:right;font-size:13px" href="/ui/changes">View all</a></h3><ul class="list">{{range .Changes}}<li><div><a href="/ui/changes/{{changeNumberOrID .}}"><b>{{changeNumberOrID .}}</b></a><div class="small">{{get . "applicationName"}}</div></div><span class="badge {{badgeClass (get . "runtimeStatus")}}">{{get . "runtimeStatus"}}</span></li>{{end}}</ul></div></div><div>{{template "changeCard" .}}</div><div><div class="card timeline"><h3>Workflow Change</h3>{{range .Events}}<div class="step"><span class="circle done"></span><div><b>{{eventStep .}}</b><div class="small">{{get . "createdAt"}}</div></div></div>{{else}}<div class="small">No events available</div>{{end}}</div><div class="card evidence" style="margin-top:18px"><h3>Available evidence</h3>{{range .Evidence}}<div class="ev-row"><div><b>{{get . "name"}}</b><div class="small">{{get . "summary"}}</div></div><a href="/ui/changes/{{get . "changeNumber"}}/evidence">View</a></div>{{else}}<div class="small">No evidence available</div>{{end}}</div></div></div>{{end}}
{{define "changeCard"}}<div class="card detail">{{if .SelectedChange}}<div class="detail-head"><h3>Change Request: {{changeNumberOrID .SelectedChange}}</h3><span class="badge {{badgeClass (get .SelectedChange "runtimeStatus")}}">{{get .SelectedChange "runtimeStatus"}}</span></div><div class="kv"><div><div class="label">Application</div><b>{{get .SelectedChange "applicationName"}}</b></div><div><div class="label">Requester</div><b>{{get .SelectedChange "requestedBy"}}</b></div><div><div class="label">Environment</div><b>{{get .SelectedChange "targetEnvironment"}}</b></div><div><div class="label">Lifecycle status</div><span class="badge {{badgeClass (get .SelectedChange "status")}}">{{get .SelectedChange "status"}}</span></div><div><div class="label">Runtime Status</div><span class="badge {{badgeClass (get .SelectedChange "runtimeStatus")}}">{{get .SelectedChange "runtimeStatus"}}</span></div><div><div class="label">Created at</div><b>{{get .SelectedChange "createdAt"}}</b></div></div><div class="section"><h3>Description</h3><p>{{get .SelectedChange "description"}}</p><div class="actions"><a class="btn" href="/ui/changes/{{changeNumberOrID .SelectedChange}}/evidence">View evidence</a><a class="btn" href="/ui/changes/{{changeNumberOrID .SelectedChange}}/events">View audit events</a></div></div><div class="section"><h3>Technical actions</h3><div class="action-groups"><div><div class="small" style="margin-bottom:8px">Recommended next actions</div><div class="actions">{{range recommendedActions .SelectedChange}}<div class="action-card"><form method="post" action="/ui/changes/{{changeNumberOrID $.SelectedChange}}/actions/{{get . "name"}}"><button class="btn {{if get . "primary"}}primary{{end}}">{{get . "label"}}</button></form><div class="small action-desc">{{get . "description"}}</div></div>{{else}}<div class="small">No recommended technical action for the current state.</div>{{end}}</div></div><details><summary>Advanced/manual actions</summary><div class="actions" style="margin-top:10px">{{range advancedActions .SelectedChange}}<div class="action-card"><form method="post" action="/ui/changes/{{changeNumberOrID $.SelectedChange}}/actions/{{get . "name"}}"><button class="btn">{{get . "label"}}</button></form><div class="small action-desc">{{get . "description"}}</div></div>{{end}}</div></details></div></div>{{with latestEvidence .Evidence}}<div class="section"><h3>Latest runtime evidence</h3><div class="small">{{get . "summary"}}</div><div class="evidence-grid">{{with get (kubeSummary .) "deployment"}}<div class="evidence-card"><h4>Deployment</h4><div class="evidence-kv"><span>Name</span><b>{{get . "name"}}</b></div><div class="evidence-kv"><span>Namespace</span><b>{{get . "namespace"}}</b></div><div class="evidence-kv"><span>Ready</span><b>{{get . "readyReplicas"}}/{{get . "desiredReplicas"}}</b></div><div class="evidence-kv"><span>Available</span><b>{{get . "availableReplicas"}}</b></div><div class="evidence-kv"><span>Updated</span><b>{{get . "updatedReplicas"}}</b></div></div>{{end}}{{with get (kubeSummary .) "service"}}<div class="evidence-card"><h4>Service</h4><div class="evidence-kv"><span>Name</span><b>{{get . "name"}}</b></div><div class="evidence-kv"><span>Type</span><b>{{get . "type"}}</b></div><div class="evidence-kv"><span>Cluster IP</span><b>{{get . "clusterIP"}}</b></div></div>{{end}}{{with get (kubeSummary .) "route"}}<div class="evidence-card"><h4>Route</h4><div class="evidence-kv"><span>Host</span><b>{{get . "host"}}</b></div><div class="evidence-kv"><span>TLS</span><b>{{get . "tlsTermination"}}</b></div><div class="evidence-kv"><span>To</span><b>{{get . "to"}}</b></div></div>{{end}}{{with get (kubeSummary .) "pods"}}<div class="evidence-card"><h4>Pods</h4><ul class="pod-list">{{range .}}<li><b>{{get . "name"}}</b> - {{get . "phase"}}, ready={{get . "ready"}}, restarts={{get . "restartCount"}}, node={{get . "nodeName"}}</li>{{end}}</ul></div>{{end}}</div><details><summary>View raw Kubernetes evidence</summary><pre class="json">{{jsonPretty (kubeSummary .)}}</pre></details></div>{{end}}{{else}}<p>No ChangeRequest available.</p>{{end}}</div>{{end}}
