...
{{define "dashboard"}}<div class="cards"><div class="card metric"><div class="icon" style="background:#dbeafe;color:#2563eb">□</div><div><div class="kpi-title-line"><span class="kpi-title">Applications</span><b class="kpi-counter">{{get .Stats "applications"}}</b></div><span>Managed total</span></div></div><div class="card metric"><div class="icon" style="background:#dcfce7;color:#16a34a">✓</div><div><div class="kpi-title-line"><span class="kpi-title">Completed changes</span><b class="kpi-counter">{{get .Stats "completed"}}</b></div><span>Last 30 days</span></div></div><div class="card metric"><div class="icon" style="background:#fef3c7;color:#d97706">◷</div><div><div class="kpi-title-line"><span class="kpi-title">Running changes</span><b class="kpi-counter">{{get .Stats "running"}}</b></div><span>Currently running</span></div></div><div class="card metric"><div class="icon" style="background:#fee2e2;color:#dc2626">!</div><div><div class="kpi-title-line"><span class="kpi-title">Failed changes</span><b class="kpi-counter">{{get .Stats "failed"}}</b></div><span>Last 30 days</span></div></div><div class="card metric"><div class="icon" style="background:#ede9fe;color:#7c3aed">▤</div><div><div class="kpi-title-line"><span class="kpi-title">Collected evidence</span><b class="kpi-counter">{{get .Stats "evidence"}}</b></div><span>For selected change</span></div></div></div><div class="grid"><div><div class="card panel"><h3>Applications <a style="float:right;font-size:13px" href="/ui/applications">View all</a></h3><ul class="list">{{range .Applications}}<li><div><a href="/ui/applications/{{get . "name"}}"><b>{{get . "name"}}</b></a><div class="small">{{get . "targetNamespace"}}</div></div><span class="badge {{badgeClass (get . "healthStatus")}}">{{get . "healthStatus"}}</span></li>{{end}}</ul></div><div class="card panel" style="margin-top:18px"><h3>Recent changes <a style="float:right;font-size:13px" href="/ui/changes">View all</a></h3><ul class="list">{{range recentChanges .Changes}}<li><div><a href="/ui/changes/{{changeNumberOrID .}}"><b>{{changeNumberOrID .}}</b></a><div class="small">{{get . "applicationName"}} · {{get . "requestedBy"}}</div></div><span class="badge {{badgeClass (get . "runtimeStatus")}}">{{get . "runtimeStatus"}}</span></li>{{end}}</ul></div></div><div>{{template "changeCard" .}}</div><div><div class="card timeline"><h3>Workflow Change</h3>{{range .Events}}<div class="step"><span class="circle done"></span><div><b>{{eventStep .}}</b><div class="small">{{get . "createdAt"}}</div></div></div>{{else}}<div class="small">No events available</div>{{end}}</div><div class="card evidence" style="margin-top:18px"><h3>Available evidence</h3>{{range .Evidence}}<div class="ev-row"><div><b>{{get . "name"}}</b><div class="small">{{get . "summary"}}</div></div><a href="/ui/changes/{{get . "changeNumber"}}/evidence">View</a></div>{{else}}<div class="small">No evidence available</div>{{end}}</div></div></div>{{end}}
