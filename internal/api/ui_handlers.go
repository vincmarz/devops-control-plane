...
func (h *Handler) uiChangeDetail(w http.ResponseWriter, r *http.Request) {
	...
	h.renderUI(w, http.StatusOK, uiData{Title: fmt.Sprintf("Change Request: %s", str(get(selected, "changeNumber"))), Subtitle: "Operational change detail", Active: "changes", SelectedChange: selected, Events: events, Evidence: evidence, Stats: map[string]any{"evidence": len(evidence)}})
}
