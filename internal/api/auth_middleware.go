...
func requiredRolesForAction(action string) ([]string, bool) {
	switch action {
	case "validate", "start-build", "check-build", "update-gitops", "check-validation", "check-deployment", "collect-evidence", "create-branch", "update-files", "open-merge-request", "sync":
		return []string{"operator", "admin"}, true
