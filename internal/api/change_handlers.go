...
func (h *Handler) checkBuild(w http.ResponseWriter, r *http.Request, id string) {
	...
	if err != nil {
		writeError(w, http.StatusUnprocessableEntity, APIError{Code: "TEKTON_CHECK_BUILD_FAILED", Message: "Unable to check Tekton application build for ChangeRequest", TechnicalMessage: err.Error(), Recoverable: true}, nil)
		return
	}
	writeJSON(w, http.StatusAccepted, result, map[string]any{"requestId": requestIDFromContext(r.Context())})
}
